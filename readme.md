...
The STM32 and ESP32 communicate using a custom protocol:

**Command Frame Format:**
```
[SYN] [CMD] [DATA_1] [DATA_1_COPY] [DATA_2] [DATA_2_COPY] ...
```

**Response Frame Format:**
```
[ACK/NAK] [LEN_LOW] [LEN_HIGH] [CRC_4_BYTES] [DATA...]
```

**Key Commands:**
- `_CMD_PING`: Connection test
- `_CMD_GET_LASTEST_VERSION_ID`: Query latest firmware version
- `_CMD_GET_UPDATE`: Request firmware chunk
- `_CMD_LOG`: Debug logging from STM32 to ESP32

**Data Integrity:**
- All data is sent twice for integrity verification
- CRC-32 checksums verify all data transfers
- Commands use ACK/NAK acknowledgments

## Troubleshooting

### Bootloader Issues

**Problem:** Device doesn't boot into application
- **Check:** Verify application is flashed at correct address (0x08008000)
- **Check:** Ensure application linker script uses correct FLASH origin
- **Check:** Verify firmware version info is valid in flash

**Problem:** Factory recovery doesn't work
- **Check:** Server is running and accessible from ESP32
- **Check:** WiFi credentials are correct in ESP32 firmware
- **Check:** Factory firmware exists at `deviceModels/<modelId>/1/application.bin`
- **Check:** ESP32 can resolve server IP and has valid SSL certificate

### Server Issues

**Problem:** Server won't start
- **Check:** Port 443 requires root privileges (use port 8443 for testing)
- **Check:** SSL certificates exist in `cert/` directory
- **Check:** Python dependencies are installed: `pip install -r requirements.txt`

**Problem:** Firmware not found (404 errors)
- **Check:** Binary file is named exactly `application.bin`
- **Check:** Directory structure matches: `deviceModels/<modelId>/<versionId>/application.bin`
- **Check:** Folder names are integers matching device model and version IDs

### ESP32 Issues

**Problem:** ESP32 won't connect to WiFi
- **Check:** SSID and password are correct in `configuration.h`
- **Check:** WiFi network is 2.4GHz (ESP32 doesn't support 5GHz)
- **Check:** Monitor output for connection status

**Problem:** HTTPS connection fails
- **Check:** Server CA certificate is embedded in ESP32 firmware
- **Check:** Server IP address is correct
- **Check:** Server is reachable from ESP32's network
- **Check:** Firewall allows HTTPS connections on port 443/8443

### Communication Issues

**Problem:** STM32 and ESP32 not communicating
- **Check:** UART connections are correct (TX↔RX, RX↔TX, GND↔GND)
- **Check:** Baud rate matches on both devices (default: 115200)
- **Check:** Both devices are powered and running

## Security Considerations

⚠️ **This is an educational project. For production use, consider the following:**

1. **Certificate Management:**
   - Use properly signed certificates from a trusted CA
   - Implement certificate rotation and expiration handling
   - Store private keys securely

2. **Authentication:**
   - Implement device authentication on the server
   - Use unique device credentials
   - Implement rate limiting on API endpoints

3. **Firmware Integrity:**
   - Sign firmware binaries with cryptographic signatures
   - Verify signatures in bootloader before installing
   - Use secure boot features if available

4. **Network Security:**
   - Use WPA2/WPA3 for WiFi connections
   - Implement VPN or secure tunneling
   - Protect against man-in-the-middle attacks

5. **Rollback Protection:**
   - Implement version rollback prevention
...
```
STM32_OTA_updater/
├── bootloader/              # STM32 bootloader firmware
│   └── Core/
│       ├── Inc/             # Header files
│       └── Src/             # Source files (main.c, etc.)
├── updaterDevice/           # ESP32 WiFi bridge firmware
│   ├── main/                # ESP32 application code
│   ├── docker-compose.yml   # Docker configuration
│   └── readme.md            # ESP32 setup instructions
├── updaterServer/           # Python HTTPS update server
│   ├── cert/                # SSL certificates (generated)
│   ├── deviceModels/        # Firmware repository (user created)
│   ├── server.py            # FastAPI server implementation
│   ├── requirements.txt     # Python dependencies
│   └── README.md            # Server setup instructions
├── diagram.png              # System architecture diagram
├── table.png                # Memory layout diagram
└── readme.md                # This file
```
...
1. **Testing Updates:**
   - Start with small test binaries to verify the update process
   - Monitor both STM32 and ESP32 serial outputs during updates
   - Use version numbers systematically (1, 2, 3, ...)

2. **Debugging:**
   - Enable debug logs in bootloader (`println` statements)
   - Monitor ESP32 output for HTTP request/response details
   - Check server logs for incoming requests

3. **Flash Management:**
   - Be careful with flash sector boundaries
   - Always verify erased regions before writing
   - Keep bootloader and application memory regions strictly separated

4. **Power Management:**
   - Ensure stable power during flash write operations
...
- Support for additional STM32/ESP32 variants
- Security enhancements

## References

- [STM32 Flash Memory Documentation](https://www.st.com/resource/en/reference_manual/rm0090-stm32f405415-stm32f407417-stm32f427437-and-stm32f429439-advanced-armbased-32bit-mcus-stmicroelectronics.pdf)
...
This project demonstrates key concepts in embedded systems:
...
**Note:** This is an educational demonstration. For production deployments, implement proper security measures, error handling, and testing procedures.
